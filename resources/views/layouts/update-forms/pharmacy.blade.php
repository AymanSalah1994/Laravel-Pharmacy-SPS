<div class="card card-primary">
    ...
    <form method="POST" action="{{ route('pharmacies.update' , $pharmacy->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <input type="text" class="form-control" name="id" id="exampleFormControlInput1" value="{{ $pharmacy->id }}"
            hidden>
        <input type="text" class="form-control" name="userable_id" id="exampleFormControlInput1"
            value="{{ $userPharma->userable_id }}" hidden>
                ...
                @if (Auth::user()->hasRole('admin'))
                    ...
                    <div class="form-group col-6">
                        ...
                        <select name="area_id" class="form-control"
                            value="@if ($pharmacy->area != null) {{ $pharmacy->area->name }}  @else Not Found @endif">
        ...
        @else
            ...
            <div class="form-group col-6">
                <label for="exampleInputEmail1">Priority</label>
                <input type="text" class="form-control" id="exampleInputEmail1" name="priority"
                    value="{{ $pharmacy->priority }}" hidden>
                <input type="text" class="form-control" id="exampleInputEmail1" name="priority"
                    value="{{ $pharmacy->priority }}" disabled>
                @error('priority')
                    <p class="text-danger mt-1">{{ $message }}</p>
                @enderror
            </div>
